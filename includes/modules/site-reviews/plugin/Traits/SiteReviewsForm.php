...
trait SiteReviewsForm
{
	...
	public function renderRequireLogin()
	{
		...
		$login = sprintf( __( 'Du musst %s sein, um eine Bewertung abzusenden.', 'blogs-directory' ),
			sprintf( '<a href="%s">%s</a>', wp_login_url( (string) get_permalink() ), __( 'eingeloggt', 'blogs-directory' ))
		);
		if( get_option( 'users_can_register' ) && glsr_get_option( 'general.require.login_register' ) == 'yes' ) {
			$login .= ' '.sprintf( __( 'Du kannst auch %s.', 'blogs-directory' ),
				sprintf( '<a href="%s">%s</a>', wp_registration_url(), __( 'ein Konto registrieren', 'blogs-directory' ))
			);
		}
